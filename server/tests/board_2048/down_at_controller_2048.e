...
feature -- Test routines
	...
	move_down_colapsing_cells_with_same_and_different_values
		...
		do
			...
			board.set_cell (4, 3, 8)

			board.down

			assert ("[3,1] has a 4", board.elements.item(3,1).value=4)
			assert ("[4,1] has a 32", board.elements.item(4,1).value=32)
			assert ("[2,2] has an 8", board.elements.item(2,2).value=8)
			assert ("[3,2] has a 16", board.elements.item(3,2).value=16)
			assert ("[4,2] has a 4", board.elements.item(4,2).value=4)
			assert ("[2,3] has an 8", board.elements.item(2,3).value=8)
			assert ("[3,3] has an 8", board.elements.item(3,3).value=8)
			assert ("[4,3] has an 8", board.elements.item(4,3).value=8)
			assert ("board has 9 filled cells", board.nr_of_filled_cells=9)
		end
	...
	move_down_colapsing_cells_with_different_values
		...
		do
			...
			assert ("[3,4] has a 2", board.elements.item(3,4).value=2)
			assert ("[4,4] has a 4", board.elements.item(4,4).value=4)
			assert ("board has 3 filled cells", board.nr_of_filled_cells=3)
		end
	...
	move_down_colapsing_cells_with_same_values
		...
		do
			...
			assert ("[4,4] has a 4", board.elements.item(4,4).value=4)
			assert ("board has 2 filled cells", board.nr_of_filled_cells=2)
		end
	...
	move_down_movement_of_cells
		...
		do
			...
			assert ("[4,1] has a 2", board.elements[4,1].value=2)
			assert ("[4,2] has a 2", board.elements[4,2].value=2)
			assert ("board has 3 filled cells", board.nr_of_filled_cells=3)
		end
	...
	move_down_error
		...
		do
			if not second_time then
				...
				board.down -- Must throw an exception
				failed := False
			end
